...
#'
#' @return a character vector (if \code{asvector == TRUE}) with ~70 main components (e.g., TestID, TestPattern, ...) extracted from the XML and all points if the TestPattern is recognized; or (if \code{asvector == FALSE}) a character matrix with one column for each point and main information as attributes.
#' @export
#'
#' @examples
...
xml_extract_full <- function(
  ...
  retval <- if (isTRUE(asvector)) {
    ...
    attr(pointmatrix, "SummaryInfo") <- maininfo
